{
  ...
  '#with_app_id':: "The Application's ID the user custom schema property should be assigned to.",
  with_app_id(value):: (
  ...
  '#with_master':: 'Master priority for the user schema property. It can be set to `PROFILE_MASTER` or `OKTA`',
  with_master(value):: (
    local converted = value;
  ...
  '#with_scope':: 'determines whether an app user attribute can be set at the Personal `SELF` or Group `NONE` level. Default value is `NONE`.',
  with_scope(value):: (
    local converted = value;
  ...
  '#with_union':: 'If `type` is set to `array`, used to set whether attribute value is determined by group priority `false`, or combine values across groups `true`. Can not be set to `true` if `scope` is set to `SELF`.',
  with_union(value):: (
    local converted = value;
