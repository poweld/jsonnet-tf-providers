{
  ...
  '#with_id':: 'ID of the log stream to retrieve, conflicts with `name`.',
  with_id(value):: (
  ...
  '#with_name':: 'Unique name for the Log Stream object, conflicts with `id`.',
  with_name(value):: (
    local converted = value;
  ...
  '#with_status':: 'Log Stream Status - can either be ACTIVE or INACTIVE only',
  with_status(value):: (
    local converted = value;
