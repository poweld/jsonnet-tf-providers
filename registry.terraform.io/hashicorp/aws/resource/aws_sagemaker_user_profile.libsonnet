{
  local block = self,
  new(domain_id, user_profile_name):: (
    {}
    + block.with_domain_id(domain_id)
    + block.with_user_profile_name(user_profile_name)
  ),
  with_arn(value):: (
  ...
  with_single_sign_on_user_identifier(value):: (
    local converted = value;
    assert std.isString(converted) : '"single_sign_on_user_identifier" expected to be of type "string"';
    {
      single_sign_on_user_identifier: converted,
    }
  ),
  with_single_sign_on_user_value(value):: (
    local converted = value;
    assert std.isString(converted) : '"single_sign_on_user_value" expected to be of type "string"';
    {
      single_sign_on_user_value: converted,
    }
  ),
  with_tags(value):: (
    local converted = value;
