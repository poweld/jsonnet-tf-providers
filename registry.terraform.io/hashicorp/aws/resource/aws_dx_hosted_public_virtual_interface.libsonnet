{
  local block = self,
  new(address_family, bgp_asn, connection_id, name, owner_account_id, route_filter_prefixes, vlan):: (
    {}
    ...
    + block.with_route_filter_prefixes(route_filter_prefixes)
  ...
  with_route_filter_prefixes(value):: (
  ...
  with_route_filter_prefixes_mixin(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert (std.isArray(converted) && std.length(std.set(converted)) == std.length(converted)) : '"route_filter_prefixes" expected to be of type "set"';
    {
      route_filter_prefixes+: converted,
