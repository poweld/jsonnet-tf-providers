{
  local block = self,
  new(infrastructure_configuration_arn, name):: (
    {}
  ...
  schedule:: {
    ...
    with_pipeline_execution_start_condition(value):: (
      local converted = value;
      assert std.isString(converted) : '"pipeline_execution_start_condition" expected to be of type "string"';
      {
        pipeline_execution_start_condition: converted,
      }
    ),
