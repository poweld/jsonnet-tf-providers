{
  local block = self,
  new(gateway_arn, network_interface_id, target_name, volume_size_in_bytes):: (
    {}
    + block.with_gateway_arn(gateway_arn)
    + block.with_network_interface_id(network_interface_id)
    + block.with_target_name(target_name)
    + block.with_volume_size_in_bytes(volume_size_in_bytes)
  ),
  with_arn(value):: (
  ...
  with_volume_arn(value):: (
    local converted = value;
    assert std.isString(converted) : '"volume_arn" expected to be of type "string"';
    {
      volume_arn: converted,
    }
  ),
