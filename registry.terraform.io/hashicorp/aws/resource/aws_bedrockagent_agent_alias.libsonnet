{
  local block = self,
  new(agent_alias_name, agent_id):: (
    {}
    + block.with_agent_alias_name(agent_alias_name)
    + block.with_agent_id(agent_id)
  ),
  with_agent_alias_arn(value):: (
  ...
  with_agent_alias_id(value):: (
    local converted = value;
    assert std.isString(converted) : '"agent_alias_id" expected to be of type "string"';
    {
      agent_alias_id: converted,
    }
  ),
  with_agent_alias_name(value):: (
    local converted = value;
