{
  ...
  with_domain_validation_records(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert (std.isArray(converted) && std.length(std.set(converted)) == std.length(converted)) : '"domain_validation_records" expected to be of type "set"';
    {
      domain_validation_records: converted,
    }
  ),
  with_domain_validation_records_mixin(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert (std.isArray(converted) && std.length(std.set(converted)) == std.length(converted)) : '"domain_validation_records" expected to be of type "set"';
    {
      domain_validation_records+: converted,
