{
  local block = self,
  new(extension_arn, resource_arn):: (
    {}
    + block.with_extension_arn(extension_arn)
  ...
  with_extension_version(value):: (
    local converted = value;
    assert std.isNumber(converted) : '"extension_version" expected to be of type "number"';
    {
      extension_version: converted,
    }
  ),
  with_id(value):: (
    local converted = value;
