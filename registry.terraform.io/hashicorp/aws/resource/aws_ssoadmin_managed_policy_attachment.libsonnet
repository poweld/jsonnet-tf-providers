{
  local block = self,
  new(instance_arn, managed_policy_arn, permission_set_arn):: (
    {}
    + block.with_instance_arn(instance_arn)
    + block.with_managed_policy_arn(managed_policy_arn)
  ...
  with_managed_policy_name(value):: (
    local converted = value;
    assert std.isString(converted) : '"managed_policy_name" expected to be of type "string"';
    {
      managed_policy_name: converted,
    }
  ),
