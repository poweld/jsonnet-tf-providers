{
  local block = self,
  new(language_code, vocabulary_name):: (
    {}
    + block.with_language_code(language_code)
    + block.with_vocabulary_name(vocabulary_name)
  ),
  with_arn(value):: (
  ...
  with_phrases(value):: (
    ...
    assert std.isArray(converted) : '"phrases" expected to be of type "list"';
    {
      phrases: converted,
    }
  ),
  with_phrases_mixin(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert std.isArray(converted) : '"phrases" expected to be of type "list"';
    {
      phrases+: converted,
  ...
  with_vocabulary_file_uri(value):: (
    local converted = value;
    assert std.isString(converted) : '"vocabulary_file_uri" expected to be of type "string"';
    {
      vocabulary_file_uri: converted,
    }
  ),
