{
  local block = self,
  new(source_region, source_snapshot_id):: (
    {}
    + block.with_source_region(source_region)
    + block.with_source_snapshot_id(source_snapshot_id)
  ),
  with_arn(value):: (
  ...
  with_completion_duration_minutes(value):: (
    local converted = value;
