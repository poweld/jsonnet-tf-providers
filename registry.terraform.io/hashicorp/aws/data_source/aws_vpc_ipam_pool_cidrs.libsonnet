{
  ...
  with_ipam_pool_cidrs(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert (std.isArray(converted) && std.length(std.set(converted)) == std.length(converted)) : '"ipam_pool_cidrs" expected to be of type "set"';
    {
      ipam_pool_cidrs: converted,
    }
  ),
  with_ipam_pool_cidrs_mixin(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert (std.isArray(converted) && std.length(std.set(converted)) == std.length(converted)) : '"ipam_pool_cidrs" expected to be of type "set"';
    {
      ipam_pool_cidrs+: converted,
