{
  ...
  with_filter_in_arns(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert std.isArray(converted) : '"filter_in_arns" expected to be of type "list"';
    {
  ...
  with_network_insights_analysis_id(value):: (
    local converted = value;
    assert std.isString(converted) : '"network_insights_analysis_id" expected to be of type "string"';
    {
      network_insights_analysis_id: converted,
    }
  ),
