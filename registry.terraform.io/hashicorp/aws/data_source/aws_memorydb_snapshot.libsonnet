{
  ...
  with_cluster_configuration(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert std.isArray(converted) : '"cluster_configuration" expected to be of type "list"';
    {
      cluster_configuration: converted,
    }
  ),
  with_cluster_configuration_mixin(value):: (
    local converted = if std.isArray(value) then value else [value];
    assert std.isArray(converted) : '"cluster_configuration" expected to be of type "list"';
    {
      cluster_configuration+: converted,
